...
#define LV_OBJ_EVENT_CB(Class, Method)                            \
    [](lv_event_t* e) {                                           \
        static_cast<Class*>(lv_event_get_user_data(e))->Method(); \
    }

#define FREERTOS_TASK_CB(Class, Method)           \
    [](void* parameter) {                         \
...
static std::string fa(uint32_t cp) {
    ...
    if (cp <= 0x7FU) {
        ...
        if (cp >= 0xD800U && cp <= 0xDFFFU) return s;  // surrogate — invalid đứng một mình
