...
class MicroSDClass : public ModuleOnce {
public:
    ...
    bool lv_read_bmp_dsc_rgb565(const char* path, lv_image_dsc_t* out_dsc, uint16_t** out_pixels) {
        if (!mounted || !path || !out_dsc || !out_pixels) return false;
        ...
        *out_pixels = nullptr;
        ...
        if (!buf) buf = (uint16_t*)heap_caps_malloc(bytes_needed, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        if (!buf) {
            f.close();
            return false;
        }
...
protected:
    void setup_impl() override { mount(); }
